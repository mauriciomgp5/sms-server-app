...
class SmsBalance extends BaseWidget
{
    ...
    {
        return [
            Stat::make('Enviados', SmsLog::where(fn ($query) => $query->where('status', StatusEnum::Sent->value)->orWhere('status', StatusEnum::Delivered))->where('company_id', session('company')?->id)->count()),
